...
R is an interpreted language, meaning commands are executed immediately as they are entered, allowing users to interact with the language and see results in real time.

Interact execution means:
...
Contrast with typical compiled languages

In many compiled languages (like C or Java):

1. You write the entire program
2. Compile the whole program
3. Run it
4. Only then see results or errors
You cannot easily run line-by-line commands in the same way.
...
complex <- c(9 + 3i) # where "i" is the imaginary part
...
numbers[-1:-4] # excess all elements except 2nd 
...
They store values as integers internally, but display meaningful labels called *levels*.
