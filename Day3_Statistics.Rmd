...
- **Numeric** – continuous or discrete values  
...
Before we can do statistics meaningfully, we need a basic understanding of probability, because:

- Statistics is built on probability.
- Probability helps us describe randomness, variation, and uncertainty in data.

In other words:

Probability explains why results vary

Statistics helps us make decisions despite that variation
...
Each sample will have a different number of males and females — this is sampling variation.
...
hist(sample_gender_counts,
  main = "Distribution of Male Students in Random Samples of 10",
  xlab = "Number of Males",
  col = "lightblue",
  breaks = 0:10)

abline(v = mean(sample_gender_counts), col = "red", lwd = 2)
...
Probability tells us how likely an event is in repeated experiments.

```{r}
# Probability that a random sample of 10 students has exactly 5 males

mean(sample_gender_counts == 5)

mean(sample_gender_counts == 2)

# If you repeat sampling many times, this fraction approximates the probability.
# This is exactly what a p-value measures in hypothesis testing.

```


Variation in results does not always imply a real difference — it may be due to chance.

### Connecting to Statistics

Once we understand variation and probability:

We can estimate population proportions (e.g., % male/female in class).

We can test hypotheses (e.g., is the class gender ratio equal?).

```{r}

# One-sample proportion test: Is the proportion of males 0.5?

prop.test(sum(students == "M"), length(students), p = 0.5)

# The p-value tells us if the observed proportion of males significantly differs from 50%.
```
...
- p-value: probability of observing the data or statistics assuming H₀ is true
...
⚠️ Statistical significance does not always imply practical importance.
...
| Key questions | “Is this effect real?” | “How well does this predict?” |
...
- **Root Mean Squared Error (RMSE)** – error on the original scale
